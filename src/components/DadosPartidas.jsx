...
function DadosPartidas() {
  ...
  function clearDates() {
    ...
    if (inputRefIni.current && inputRefFin.current) {
        inputRefIni.current.value = '';
        inputRefFin.current.value = '';
    }
  ...
  useEffect(() => {
    ...
    onValue(partidaRef, (snapshot) => {
      ...
      const partidasComID = Object.entries(data).map(
        ([partidaId, partidaData]) => {
          return { id: partidaId, ...partidaData };
        }
      );

      let partidasFiltradas = partidasComID.filter(
      ...
      if (pesqP1.trim() !== "") {
        partidasFiltradas = partidasFiltradas.filter((partida) =>
          partida.player1.nome.toLowerCase().includes(pesqP1.toLowerCase())
        );
      }

      if (pesqP2.trim() !== "") {
        partidasFiltradas = partidasFiltradas.filter((partida) =>
          partida.player2.nome.toLowerCase().includes(pesqP2.toLowerCase())
        );
      }
            ...
            {partidasUsuario.map((partida, index) => (
              <tbody key={index}>
                <tr className="text-center">
                  ...
                  <td>
                    {parseInt(partida.player1.qteGols, 10) >
                    parseInt(partida.player2.qteGols, 10) ? (
                      <img
                        style={{ width: 30 }}
                        src={p1Win}
                        alt="Player 1 Wins"
                      ...
                      <img style={{ width: 30 }} src={emp} alt="Draw" />
                    )}
                  </td>
                  <td>
      ...
      <div className="d-lg-flex justify-content-center gap-5 m-2">
        <div className="d-flex flex-column card mt-3 p-3 p-md-5 bg-dark text-light">
          ...
          <div className="d-md-flex gap-4 py-2 justify-content-center align-items-end">
            ...
            <button
              style={{ maxWidth: 80 }}
              className="w-50 btn btn-danger mt-2 mt-md-0"
              onClick={clearDates}
            >
